...
public final class JDBCConnectionAdvice {
    ...
    public static ElementMatcher<? super MethodDescription> getMatcher(String module) {
        ElementMatcher.Junction<? super MethodDescription> statementMatcher = isDeclaredBy(
                        nameContainsIgnoreCase(module).and(isSubTypeOf(Statement.class)))
                .and(named("addBatch")
                        .or(named("execute"))
        ...
        ElementMatcher.Junction<? super MethodDescription> connectionMatcher = isDeclaredBy(
                        nameContainsIgnoreCase(module).and(isSubTypeOf(Connection.class)))
                .and(named("prepareStatement").or(named("prepareCall")).or(named("nativeSQL")));
