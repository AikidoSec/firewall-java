...
public final class HostnameCollector {
    ...
    public static void report(String hostname, InetAddress[] inetAddresses) {
        try {
            ...
            // Currently using hostnames from thread cache, might not be as accurate as using Context-dependant
            // hostnames.
            if (ThreadCache.get() == null || ThreadCache.get().getHostnames() == null) {
                logger.trace("Thread cache is empty, returning.");
                return;
            }
            for (Hostnames.HostnameEntry hostnameEntry :
                    ThreadCache.get().getHostnames().asArray()) {
                if (!hostnameEntry.getHostname().equals(hostname)) {
                    continue;
                }
                logger.debug(
                        "Hostname: %s, Port: %s, IPs: %s",
                        hostnameEntry.getHostname(), hostnameEntry.getPort(), ipAddresses);

                Attack attack = new SSRFDetector()
                        .run(
                                hostname,
                                hostnameEntry.getPort(),
                                ipAddresses,
