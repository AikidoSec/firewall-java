    ...
    public void run() {
        ...
        if (res.isPresent()) {
            ...
            if (configLastUpdatedAt.isEmpty() || configLastUpdatedAt.get() < configAccordingToCloudUpdatedAt) {
                ...
                Optional<APIResponse> newConfig = connectionManager
                        .getApi()
                        .fetchNewConfig(connectionManager.getToken(), /* Timeout in seconds: */ 3);
                ...
                Optional<ReportingApi.APIListsResponse> blockedListsRes =
                        connectionManager.getApi().fetchBlockedLists(connectionManager.getToken());
