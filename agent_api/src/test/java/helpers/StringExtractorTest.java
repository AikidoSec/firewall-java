...
public class StringExtractorTest {
    ...
    public void testExtractQueryObjects() {
        assertEquals(
                fromObj(Map.of("age", ".", "$gt", ".age", "21", ".age.$gt")),
                extractStringsFromObject(Map.of("age", Map.of("$gt", "21"))));
        assertEquals(
                fromObj(Map.of("title", ".", "$ne", ".title", "null", ".title.$ne")),
    ...
    public void testExtractCookieObjects() {
        assertEquals(
                fromObj(Map.of("session2", ".", "session", ".", "ABC", ".session", "DEF", ".session2")),
                extractStringsFromObject(Map.of("session", "ABC", "session2", "DEF")));
        assertEquals(
                fromObj(Map.of("session2", ".", "session", ".", "ABC", ".session")),
    ...
    public void testExtractHeaderObjects() {
        assertEquals(
                fromObj(Map.of("Content-Type", ".", "application/json", ".Content-Type")),
                extractStringsFromObject(Map.of("Content-Type", "application/json")));
        assertEquals(fromObj(Map.of("Content-Type", ".")), extractStringsFromObject(Map.of("Content-Type", 54321)));
        assertEquals(
                fromObj(Map.of(
                        "Content-Type",
                        ".",
    ...
    public void testExtractBodyObjects() {
        assertEquals(
                fromObj(Map.of("nested", ".nested", "$ne", ".nested.nested")),
                extractStringsFromObject(Map.of("nested", Map.of("nested", Map.of("$ne", true)))));
        assertEquals(
                fromObj(Map.of("age", ".", "$lt", ".age", "$gt", ".age", "21", ".age.$gt", "100", ".age.$lt")),
