...
public class LoggingTest {
    ...
    public void testLoggerCanHandleArraysAndCollections(StdOut out) {
        ...
        assertTrue(out.capturedString()
                .contains("INFO dev.aikido.agent_api.collectors.FileCollector: Test String here [1, 6, 2, 3]"));

        logger.info("Test %s %s", "String2", (new String[] {"Hello", "World"}));
        assertTrue(out.capturedString()
                .contains("INFO dev.aikido.agent_api.collectors.FileCollector: Test String2 [Hello, World]"));

        logger.info("Test %s %s", "String2", List.of("Hiya", "2"));
        assertTrue(out.capturedString()
                .contains("INFO dev.aikido.agent_api.collectors.FileCollector: Test String2 [Hiya,2]"));
    }
    ...
    public void testParseArgumentsWithCarriageReturn() {
        ...
        List<Object> result = logger.parseArguments(args);
        assertEquals(
                Arrays.asList("input_with_carriage_returnnext_line"),
                result,
    ...
    public void testParseArgumentsWithMixedInputIncludingSpecialChars() {
        ...
        List<Object> result = logger.parseArguments(args);
        assertArrayEquals(
                new String[] {"normal_input", "malicious_input; DROP TABLE users; --", "input_with_newlinenext_line"},
                result.toArray());
    }
    ...
    public void testParseArgumentsWithLongString() {
        ...
        List<Object> result = logger.parseArguments(args);
        assertEquals(
                Arrays.asList(longString.substring(0, MAX_ARGUMENT_LENGTH)),
                result,
                "String should be truncated to the maximum length.");
    }
    ...
    public void testParseArgumentsWithNewlineInLongString() {
        ...
        assertEquals(
                Arrays.asList(expected.substring(0, MAX_ARGUMENT_LENGTH)),
                result,
                "String should be truncated and newline should be removed.");
    }
    ...
    public void testParseArgumentsWithCarriageReturnInLongString() {
        ...
        assertEquals(
                Arrays.asList(expected.substring(0, MAX_ARGUMENT_LENGTH)),
                result,
                "String should be truncated and carriage return should be removed.");
    }
    ...
    public void testParseArgumentsWithCollectionContainingLongString() {
        ...
        List<Object> result = logger.parseArguments(args);
        assertEquals(
                Arrays.asList(("[value1," + longString + "]").substring(0, MAX_ARGUMENT_LENGTH)),
                result,
                "Collection string should be truncated to the maximum length.");
    }
    ...
    public void testParseArgumentsWithMultipleArgsIncludingLongString() {
        ...
        assertEquals(
                Arrays.asList("normal_input", longString.substring(0, MAX_ARGUMENT_LENGTH)),
                logger.parseArguments(args),
                "Should truncate the long string while keeping the normal input.");
    }
